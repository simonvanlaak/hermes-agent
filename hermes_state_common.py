...
-- Immutable per-call/per-period usage deltas. Unlike session_model_usage, these
-- rows are never updated or synthesized from cumulative totals, so consumers
-- can apply real hourly/daily windows without assigning a whole session to its
-- last_seen timestamp. Deliberately no FK: session pruning must not erase the
-- accounting ledger.

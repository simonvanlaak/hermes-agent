...
def make_tool_progress_cb(
    ...
    def _subagent_identity(kwargs: Dict[str, Any]) -> str | None:
        ...
        return f"{delegation_id}:{task_index}" if delegation_id is not None and task_index is not None else None

    def _subagent_payload(event_type: str, preview: Any, kwargs: Dict[str, Any]) -> Dict[str, Any] | None:
        subagent_id = _subagent_identity(kwargs)
        if subagent_id is None:
            return None
        payload = {
            "hermesSubagent": True,
            "subagentId": subagent_id,
            "goal": str(kwargs.get("goal") or preview or "Hermes subagent"),
            "model": kwargs.get("model"),
            "depth": kwargs.get("depth"),
            "childSessionId": kwargs.get("child_session_id"),
            "delegationId": kwargs.get("delegation_id"),
            "taskIndex": kwargs.get("task_index"),
            "taskCount": kwargs.get("task_count"),
            "event": event_type,
        }
    ...
    def _relay_subagent(event_type: str, name: Any, preview: Any, kwargs: Dict[str, Any]) -> bool:
        ...
        if tool_call_id is None:
            ...
            _send_update(
                ...
                acp.start_tool_call(
                    ...
                    title,
                    kind="think",
                    content=[acp.tool_content(acp.text_block(str(payload["goal"])))],
                    locations=[],
                    raw_input=payload,
                ),
            )
        status = str(kwargs.get("status") or "")
        terminal = event_type == "subagent.complete"
        failed = status in {"failed", "error", "timeout"}
        output = {
            "event": event_type,
            "status": ("failed" if failed else status or "completed") if terminal else "running",
            "childSessionId": kwargs.get("child_session_id"),
            "progress": str(preview or name or "") or None,
        ...
        _send_update(
            ...
            acp.update_tool_call(
                tool_call_id,
                kind="think",
                status=("failed" if failed else "completed") if terminal else "in_progress",
                content=(
                    [acp.tool_content(acp.text_block(str(output.get("summary") or output.get("progress") or "")))]
                    if terminal
                    else None
                ),
                raw_output=output,
    ...
    def _tool_progress(event_type: str, name: str = None, preview: str = None, args: Any = None, **kwargs) -> None:
        if _relay_subagent(event_type, name, preview, kwargs):
            return
